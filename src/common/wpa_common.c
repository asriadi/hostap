...
int fils_key_auth_sk(const u8 *kck, size_t kck_len, const u8 *snonce,
		     ...
		     size_t *key_auth_len)
{
	 ...
	 * Key-Auth = HMAC-Hash(KCK, SNonce || ANonce || STA-MAC || AP-BSSID
	 *                      [ || gSTA || gAP ])
	 */
	addr[0] = snonce;
	...
	if (wpa_key_mgmt_sha384(akmp)) {
		*key_auth_len = 48;
		res = hmac_sha384_vector(kck, kck_len, num_elem, addr, len,
					 key_auth_sta);
	} else if (wpa_key_mgmt_sha256(akmp)) {
		*key_auth_len = 32;
		res = hmac_sha256_vector(kck, kck_len, num_elem, addr, len,
					 key_auth_sta);
	} else {
		return -1;
	}
	 ...
	 * Key-Auth = HMAC-Hash(KCK, ANonce || SNonce || AP-BSSID || STA-MAC
	 *                      [ || gAP || gSTA ])
	 */
	addr[0] = anonce;
	addr[1] = snonce;
